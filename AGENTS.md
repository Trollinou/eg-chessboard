...
| **Jeu de Pièces**     | Prop  | `pieceSet`        | `pieceSet`            | Type `PieceSet` (`'staunton'`, `'merida'`, `'alpha'`, `'cburnett'`, `'cardinal'`, `'dubrovny'`, `'fantasy'`, `'firi'`, `'maestro'`, `'staunty'`). Applique la classe `.piece-set-${pieceSet}` sur `.main-wrap` et synchronise dynamiquement avec `BoardCore.setPieceSet()`. |
...
- `core.setPieceSet(pieceSet: PieceSet): void` : Définit dynamiquement le style de pièces utilisé (`'staunton'`, `'merida'`, `'alpha'`, `'cburnett'`, `'cardinal'`, `'dubrovny'`, `'fantasy'`, `'firi'`, `'maestro'`, `'staunty'`).
